...
rgbArray* parseUncompressedColorMap(header* headerTemp, FILE* file){
    ...
    arr->height = headerTemp->height;
    arr->width = headerTemp->width;
    arr->stride = headerTemp->colorMapDepth/8;
    ...
    if(indexWidth == 8){
        ...
        for(int i = 0, j = 0; i < dataSize; i++, j += 3){
            arr->values[j] = palette[indices[i]*3];
            arr->values[j + 1] = palette[(indices[i]*3) + 1];
            arr->values[j + 2] = palette[(indices[i]*3) + 2];
        }
        fclose(file);
        return arr;
    }
